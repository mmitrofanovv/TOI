...
m = [1 -3; -1 3; 10 -2]';   % мат. ожидания - координаты центров классов (2,-3) и (1,6)
% априорные вероятности классов (доля образов каждого класса в общей выборке)
pw = [0.3, 0.3, 0.3];    
np=sum(pw); pw=pw/np;

C = [3 1; 1 3];   % матрица ковариаций классов
%C = [7 3; 3 7]; увеличиваем дисперсию, чтобы увеличить суммарную ошибку
...
for i=1:M %цикл по классам
    ims = repmat(m(:,i), [1, Ks(i)]) + randncor(n,Ks(i),C); %генерация Ks(i) образов i-го класса
    if (n == 2)            
...
for i = 1 : M
    ...
    PIJ(i,i)=1-sum(PIJ(i,:));%нижняя граница вероятности правильного распознавания (на главной диагонали)
end
...
Pc_=Pc_/K;  % атрица ошибок, полученная экспериментально
% у нее такая же структура как и в PIJ, тока вычисляется численно, а не по
% формулам
